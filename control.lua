...
script.on_event(defines.events.on_player_created, function(event)
	...
	chkfactionghosts(player)
end)

script.on_init(function(event)
	if settings.global["TinyStart-init-spawn"].value then
...
script.on_event(defines.events.on_runtime_mod_setting_changed, function(event)
	if event.setting == "TinyStart-ghosts" then
		for index,force in pairs(game.forces) do
			if #force.players > 0 then
				if settings.global["TinyStart-ghosts"].value then
					force.ghost_time_to_live = uint_max
				else
...
function chkfactionghosts(player)
	if settings.global["TinyStart-ghosts"].value then
		local force = player.force
		-- This check might not be necessary. Mostly here just in case writing to the setting
		-- while someone has a billion ghosts might cause a performance hit
		if force.ghost_time_to_live == 0 then
			force.ghost_time_to_live = uint_max
		end
	end
end
...
function genarmor(player, chara)
	...
	if config_marathon_start then
		if config_marathon_firstonly and player.index > 1 then return end
